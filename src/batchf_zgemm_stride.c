...
/**
    Purpose
    -------
    <b>batchf_zgemm_stride</b> is a batch version of zgemm for fixed size.
    ...
    @param[in]
    strideA <tt>int</tt>.
            strideA specifies the storage spacing between A matrices.
                    strideA must be at least M by K.
    ...
    @param[in]
    strideB <tt>int</tt>.
            strideB specifies the storage spacing between A matrices.
            strideB must be at least K by N.
    ...
    @param[in]
    strideC <tt>int</tt>.
            strideC specifies the storage spacing between A matrices.
            strideC must be at least M by N.
    ...
    const int batch_count, int *info)
{
    ...
    int batch_iter;
    int LDA,  LDB;
    char func_name[15] = "batchf_zgemm_op";
    ...
    if (strideA < M*K) {
        xerbla_batch(func_name, BBLAS_ERR_STRIDEA, -1);
        *info =  BBLAS_ERR_STRIDEA;
        return;
    }
    if (strideB < K*N) {
        xerbla_batch(func_name, BBLAS_ERR_STRIDEB, -1);
        *info =  BBLAS_ERR_STRIDEB;
        return;
    }
    if (strideC < M*N) {
        xerbla_batch(func_name, BBLAS_ERR_STRIDEC, -1);
        *info =  BBLAS_ERR_STRIDEC;
        return;
    }
    ...
    for (batch_iter = 0; batch_iter < batch_count; batch_iter++)
    {
        ...
        K,
        CBLAS_SADDR(alpha),
        //&arrayA[batch_iter*M*K],
        &arrayA[batch_iter*strideA],
        lda,
        //&arrayB[batch_iter*K*N],
        &arrayB[batch_iter*strideB],
        ldb,
        CBLAS_SADDR(beta),
        //&arrayC[batch_iter*M*N],
        &arrayC[batch_iter*strideC],
        ldc);
    }
    /* Successful */
    *info = BBLAS_SUCCESS;
